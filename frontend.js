...
let userId;
let authToken; // Přidáme proměnnou pro ukládání autorizačního tokenu
...
nextButton.addEventListener('click', async () => {
    ...
    authToken = loginData.token; // Uložíme autorizační token

    // ----- Přidaný kód -----
    if (loginData.userId === userId) {
        console.log('Jste přihlášeni');
    } else {
        console.log('Nepodařilo se přihlásit');
    }
    // -----------------------
...
registerButton.addEventListener('click', async () => {
    ...
    const phone = parseInt(document.getElementById('phoneInput').value);
    ...
    const insuranceResponse = await fetch(`http://localhost:3000/api/insured/${userId}/insurances`, {
        ...
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}` // Přidáme autorizační token do hlavičky
        },
